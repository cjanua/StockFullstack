...
from stock_fullstack.common.sdk.loaders import (
    get_account,
    get_positions,
    get_history,
    get_trading_client,
...
async def get_optimal_portfolio(lookback_days: int = 365):
    ...
    cached_get_optimal_portfolio(lookback_days)
...
async def clear_history_cache_endpoint(days: int = None):
    ...
    from stock_fullstack.common.sdk.loaders import clear_history_cache

    try:
        await asyncio.to_thread(clear_history_cache, days)
        return {
            "status": "success",
            "message": f"Historical data cache {'for ' + str(days) + ' days ' if days else ''}cleared successfully"
        }
    except Exception as e:
